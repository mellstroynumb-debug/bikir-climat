...
function formatOrderMessage(
  ...
  const clientDetails = [
    `*Клиент:* ${escapeMarkdownV2(order.customer_name || order.customerName || 'Не указано')}`,
    `*Телефон:* \`${escapeMarkdownV2(order.phone || 'Не указано')}\``,
  ]

  if (type !== 'callback') {
    const itemLines = items
      .map(item => `\\- ${escapeMarkdownV2(item.title)} \\(x${item.quantity}\\)`)
      .join('\n')
    ...
    const orderDetails = [
      ...
      `*Товары:*\n${itemLines}`,
      `\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-`,
      `*Общая сумма:* ${new Intl.NumberFormat('ru-RU').format(totalPrice)} ${escapeMarkdownV2(order.currency || '')}`,
      `\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-\\-`,
