...
export default function CheckoutPage() {
  ...
  const onSubmit = async (formData: CheckoutFormData) => {
    ...
    try {
      ...
      const orderItems = cart.map(item => ({
        ...
        price: (region === 'PMR' ? item.price_pmr : item.price_md) || 0,
      }))
      ...
      await sendTelegramNotification(savedOrder.id, { ...newOrder, createdAt: new Date().toISOString() }, cartItemsForTelegram, 'cart')
  ...
  return (
    <div className="container mx-auto px-4 py-8 md:py-12">
      <h1 className="text-2xl md:text-3xl font-bold font-sans mb-8">{'Оформление заказа'}</h1>
                ...
                {cart.map((item) => {
                  ...
                  return (
                    <li key={item.id} className="flex py-4">
                      {imgSrc ? (
                        <Image src={imgSrc} alt={item.title} width={64} height={64} className="rounded-md object-cover border" />
                      ) : (
