...
/* Helper: get spec value by name */
function specVal(product: Product, name: string): string | undefined {
...
function Quiz({ allProducts }: { allProducts: Product[] }) {
  ...
  const [results, setResults] = useState<
    Record<string, { title: string; description: string; products: Product[] }>
  >({})
  ...
  const handleQuizSubmit = () => {
    ...
    const filteredByArea = allProducts.filter((p) => {
      const productArea = Number(specVal(p, 'Площадь (м²)') ?? specVal(p, 'area_sq_m')) || 0
      if (productArea === 0) return false
    ...
    const groups = {
      optimal: {
        ...
        products: [] as Product[],
        description:
          'Надежные и тихие инверторные модели. Экономят электроэнергию и создают комфортный климат.',
      },
      budget: {
        title: 'Бюджетные модели',
        products: [] as Product[],
        description: 'Отличный выбор для простого и эффективного охлаждения без лишних затрат.',
      },
      premium: {
        title: 'Премиум-класс',
        products: [] as Product[],
        description:
          'Флагманские модели от ведущих брендов с максимальным набором функций.',
      },
    }

    const priceRanges = { budget: 4500, premium: 8500 }
    const premiumBrands = ['Mitsubishi', 'Daikin', 'Cooper&Hunter', 'Gree']

    filteredByArea.forEach((p) => {
      const price = getPrice(p, region)
      if (!price) return
      const inverter = specVal(p, 'Инвертор') ?? specVal(p, 'inverter')
      ...
      } else if (inverter === 'Да' && price > priceRanges.budget) {
    ...
    if (priority === 'quiet') {
      groups.optimal.products.sort((a, b) => {
        const aInv = specVal(a, 'Инвертор') ?? specVal(a, 'inverter')
        const bInv = specVal(b, 'Инвертор') ?? specVal(b, 'inverter')
        if (aInv === 'Да' && bInv !== 'Да') return -1
        if (aInv !== 'Да' && bInv === 'Да') return 1
        return 0
      })
    }

    let finalGroups: Record<string, (typeof groups)[keyof typeof groups]> = {}
    if (budget === 'premium') {
  ...
  return (
    <section id="quiz" className="w-full scroll-mt-20">
      <Card className="max-w-3xl mx-auto overflow-hidden">
        <CardHeader className="text-center px-4 pt-6 md:p-6">
          ...
          <CardDescription className="mt-2 text-sm sm:text-base">
            Пройдите наш мини-тест и получите персональную подборку за 1 минуту.
          </CardDescription>
        </CardHeader>
            ...
            <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="text-xs sm:text-sm text-muted-foreground mt-1 text-center">
                          Для спальни, детской или кабинета
                        </span>
                      </Label>
                      <Label
                        htmlFor="power"
                      ...
                      >
                        ...
                        <span className="text-xs sm:text-sm text-muted-foreground mt-1 text-center">
                          Для гостиной, офиса или магазина
                        </span>
                      </Label>
                    </RadioGroup>
                    <div className="flex justify-end pt-4">
                      <Button onClick={handleNextStep} disabled={!priority}>
              ...
              {step === 2 && (
                ...
                >
                  <div className="space-y-6 pt-2">
                    <Label
                      htmlFor="area-slider"
                      className="text-base sm:text-lg font-semibold text-center block"
                    >
                      2. Какая площадь помещения?
                    </Label>
                    <div className="p-4">
                      <Slider
                        id="area-slider"
                      ...
                      />
                      <p className="text-center text-xl sm:text-2xl font-bold mt-4 text-primary">
                        {area[0]} м{'\u00B2'}
                      </p>
                    </div>
                ...
                >
                  <div className="space-y-4 pt-2">
                    ...
                    <RadioGroup
                      ...
                      className="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-4 pt-2"
                    >
                      <Label
                        htmlFor="eco"
                        className="flex flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground cursor-pointer [&:has([data-state=checked])]:border-primary h-24"
                      >
                    ...
                    <div className="flex justify-between items-center pt-4">
                      ...
                      <Button size="lg" onClick={handleQuizSubmit} disabled={!budget}>
                        Подобрать <Sparkles className="ml-2 h-4 w-4" />
                      </Button>
                    </div>
                  </div>
                </motion.div>
              ...
              {step === 4 && (
                <motion.div
                  ...
                  exit="exit"
                  transition={{ duration: 0.3 }}
                  className="absolute w-full flex flex-col items-center justify-center text-center h-full"
                >
                  <div className="space-y-4">
                    <CheckCircle2 className="mx-auto h-12 w-12 text-green-600" />
                    <h3 className="text-xl font-bold text-foreground">
                      Отлично! Ваша подборка готова.
                    </h3>
                    <p className="text-muted-foreground max-w-sm mx-auto">
                      Мы проанализировали ваши ответы и подобрали лучшие варианты.
                    </p>
                    <div className="pt-4">
      ...
      {submitted && (
        ...
        >
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-center mb-10 font-sans text-foreground">
          ...
          {Object.keys(results).length > 0 ? (
            <div className="space-y-12">
              {Object.entries(results).map(([key, groupData]) => (
                <div key={key}>
                  <h3 className="text-xl sm:text-2xl font-bold font-sans mb-3 text-foreground">
                    {groupData.title}
                  </h3>
                  <div className="flex items-start bg-primary/5 border-l-4 border-primary p-4 rounded-md mb-6">
                    <Lightbulb className="h-6 w-6 text-primary mr-4 mt-1 flex-shrink-0" />
                    <p className="text-sm text-foreground/80">{groupData.description}</p>
                  </div>
...
export default function Home() {
  const { data: allProducts, isLoading } = useSWR<Product[]>('/api/products', fetcher)

  const conditionerProducts = useMemo(() => {
    if (!allProducts) return []
    // Include all products (we no longer have a "service" category filter here)
    return allProducts
  }, [allProducts])
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <section id="featured-products" className="py-16 md:py-24 scroll-mt-20">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight font-sans text-foreground">
              Популярные модели
            </h2>
      ...
      <section className="py-16 md:py-24 bg-secondary/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {isLoading && !allProducts ? (
            ...
            <Quiz allProducts={conditionerProducts} />
          ) : null}
        </div>
      </section>
